...
static fat_data* g_data;        //make sure things are loaded immediately at runtime
...
fat_file* fat_open_entry(disk* dsk, fat_direntry* entry){
    int handle=-1;
    ...
    if(!disk_read_sectors(dsk, fat_clust2lba(fd->curr_clust), 1, fd->buffer)){
        ...
        printf("[fat_open_entry] Read error on drive %d, cluster=%d, lba=\n", dsk->id, fd->curr_clust, fat_clust2lba(fd->curr_clust));
        return NULL;
    }
...
uint32_t fat_next_clust(uint32_t curr_clust){
    ...
    if(curr_clust%2==0){
        return (*(uint16_t*)(g_fat+fatind)) & 0x0FFF;
    }
    return (*(uint16_t*)(g_fat+fatind))>>4;
    
}

uint32_t fat_read(disk* dsk, fat_file* file, uint32_t byte_cnt, void* out){
    //get file data
    fat_filedata* fd;
...
bool fat_read_entry(disk* dsk, fat_file* file, fat_direntry* dir_entry){
...
bool fat_find_file(disk* dsk, fat_file* file, const char* name, fat_direntry* entry_out){
    fat_direntry entry;
...
fat_file* fat_open(disk* dsk, const char* path){
    ...
    fat_file* curr=&g_data->root_dir.public;
